...
public class ShowInterestCommand extends BukkitCommand {
    ...
    public boolean execute(@NotNull CommandSender sender, @NotNull String commandLabel, @NotNull String @NotNull [] args) {
        ...
        if (!manager.hasActiveShowcase()) {
            player.sendRichMessage("<bold>[ 자랑 시스템 ] <reset><red>지금은 자랑하고 있는 사람이 없습니다 !");
            return false;
        }
        ...
        if (player.getUniqueId().equals(manager.getCurrentShowerID())) {
            ...
            Bukkit.broadcast(MiniMessage.miniMessage().deserialize(
                    "<bold>[ 자랑 시스템 ] <reset>많이 관심이 고프신가봅니다. 모두 관심을 주세요 ㅋ (/관심)"
            ));
            return false;
        }
        boolean added = manager.tryAddInterest(player);
        if (!added) {
            player.sendRichMessage("<bold>[ 자랑 시스템 ] <reset><yellow>이미 관심을 주셨습니다 !");
            return false;
        }

        Player target = Bukkit.getPlayer(manager.getCurrentShowerID());
        if (target == null) {
            sender.sendRichMessage("<bold>[ 자랑 시스템 ] <reset><red>관심 대상이 접속을 종료했거나 존재하지 않습니다.");
            return false;
        }
