...
public class GaechuCommand extends BukkitCommand {
    public GaechuCommand(@NotNull JavaPlugin plugin) {
        super("개추");
    ...
    public boolean execute(@NotNull CommandSender sender, @NotNull String commandLabel, @NotNull String @NotNull [] args) {
        ...
        if (!added) {
            player.sendRichMessage("<bold>【 자랑 】 <reset><yellow>이미 개추를 누르셨습니다 !");
            player.playSound(player.getLocation(), Sound.BLOCK_ANVIL_LAND, 1.0f, 1.0f);
            return false;
        }
        ...
        if (target == null) {
            player.sendRichMessage("<bold>【 자랑 】 <reset><red>관심 대상이 접속을 종료했거나 존재하지 않습니다.");
            player.playSound(player.getLocation(), Sound.BLOCK_ANVIL_LAND, 1.0f, 1.0f);
            return false;
        }
        Bukkit.broadcast(MiniMessage.miniMessage().deserialize(
                "<bold>【 자랑 】 <reset><light_purple><player><reset> : 개추요 ㅎ (/개추)",
